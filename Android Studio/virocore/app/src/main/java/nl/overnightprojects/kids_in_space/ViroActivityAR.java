...
public class ViroActivityAR extends Activity implements ARScene.Listener {
    ...
    private String[] spaceMetallicAssets = {"empty.png", "empty.png", "empty.png", "empty.png", "empty.png"};
    private String[] spaceNormalMapAssets = {"empty.png", "empty.png", "empty.png", "empty.png", "empty.png"};

    private enum CAR_MODEL{
        BALLOON("baloon/Baloon Texture.jpg", new Vector(19, 42, 143)),
        RULER("ruler/Ruler Texture.jpg", new Vector(19, 42, 143)),
        SHOEBOX("box/Cardboard Box Texture.jpg", new Vector(19, 42, 143)),
        CARDBOARDTUBE("cardboardtube/cardboard tube texture.jpg", new Vector(19, 42, 143)),
        GLASSES("glasses/Glasses texture.jpg", new Vector(19, 42, 143));
    ...
    private void initFireBase() {
        ...
        //databaseReference.child("NR").setValue("0");
    }

    private void getMarkerDataFromFireBase(){
        databaseReference.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                onRenderCreate(dataSnapshot.child("NR").getValue(String.class));
    ...
    private void onRenderCreate(String marker) {
        // Create the base ARScene
        mScene = new ARScene();
        mScene.setListener(this);
        mViroView.setScene(mScene);

        // Create an ARImageTarget
        Bitmap teslaLogoTargetBmp = getBitmapFromAssets("marker01.png");
     ...
     */
    private void initCarModel(Node groupNode, final String marker) {
        int index = Integer.valueOf(marker);
        String uriBase = "file:///android_asset/";
        ...
        switch (index){
            case 0:
                uriFinal = uriBase + "baloon/baloon.obj";
                break;
            case 1:
                uriFinal = uriBase + "ruler/ruler.obj";
                break;
            case 2:
                uriFinal = uriBase + "box/box.obj";
                break;
            case 3:
                uriFinal = uriBase + "cardboardtube/tubenew.obj";
                break;
            case 4:
                uriFinal = uriBase + "glasses/glassesnew.obj";
                break;
            default:
                uriFinal = uriBase + "baloon/baloon.obj";
                break;
        ...
        fbxCarNode.loadModel(Uri.parse(uriFinal), Object3D.Type.OBJ, new AsyncObject3DListener() {
            @Override
            public void onObject3DLoaded(final Object3D object, final Object3D.Type type) {
                preloadCarColorTextures(object, marker);
            }
    ...
    */
    ...
    private void initSceneLights(Node groupNode){
        ...
        Texture environment = Texture.loadRadianceHDRTexture(Uri.parse("file:///android_asset/indoor.jpg"));
        mScene.setLightingEnvironment(environment);
    }

    private Material preloadCarColorTextures(Node node, String marker){
        int index = Integer.valueOf(marker);

        final Texture metallicTexture = new Texture(getBitmapFromAssets(spaceMetallicAssets[index]),
                Texture.Format.RGBA8, true, true);
        final Texture normalMapTexture = new Texture(getBitmapFromAssets(spaceNormalMapAssets[index]),
                Texture.Format.RGBA8, true, true);
        ...
        switch (index){
            case 0:
                material.setDiffuseTexture(mCarColorTextures.get(CAR_MODEL.BALLOON));
                break;
            case 1:
                material.setDiffuseTexture(mCarColorTextures.get(CAR_MODEL.RULER));
                break;
            case 2:
                material.setDiffuseTexture(mCarColorTextures.get(CAR_MODEL.SHOEBOX));
                break;
            case 3:
                material.setDiffuseTexture(mCarColorTextures.get(CAR_MODEL.CARDBOARDTUBE));
                break;
            case 4:
                material.setDiffuseTexture(mCarColorTextures.get(CAR_MODEL.GLASSES));
                break;
            default:
                material.setDiffuseTexture(mCarColorTextures.get(CAR_MODEL.BALLOON));
                break;
